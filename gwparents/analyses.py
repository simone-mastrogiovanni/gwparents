...
class mixture_analyis(object):
    ...
    """
    def __init__(self,list_of_posterior_samples,list_of_syn_populations,montecarlo_mode,scale_free):
    ...
    def run_bilby(self,posterior_names=None,syn_names=None,pmode=None,prior_dict=None,kernel_kwargs={},**kwargs):
        """
        ...
        prior_dict: str or dict (optional)
            If string should be isotropic to indicate a prior isotropic in models in sqrt(mixing), otherwise homogeneous, otherwise a dict of priors from Bilby. In this case
        ...
        **kwargs: any kwargs to pass to bilby run_sampler
        ...
        prior_mode=list(pmode.keys())[0]
        print('Using prior mode ',prior_mode)

        if prior_mode=='dirichlet':
            Nmodels=len(self.syn_names)
            mapping={syn_name:'syn'+str(i) for i,syn_name in enumerate(self.syn_names)}
            prior_dict={}
            for i in _np.arange(2,Nmodels+1,1):
                pp=_absoluteSineNdimensional(i,Nmodels)
                prior_dict[pp.name]=pp
            for i, syn_name in enumerate(self.syn_names):
                prior_dict['alpha_syn'+str(i)]=pmode['dirichlet'][syn_name]
                prior_dict['alpha_syn'+str(i)].name='alpha_syn'+str(i)
                prior_dict['alpha_syn'+str(i)].latex_label='alpha_syn'+str(i)
        elif prior_mode=='free':
            prior_dict={var:pmode['free'][var] for var in pmode['free'].keys()}

        else:
            raise ValueError('Prior mode not recognized')
        print(prior_dict)
        res=_bilby.run_sampler(_mixture_bilby_likeli(self,prior_mode=prior_mode,scale_free=self.scale_free),prior_dict,**kwargs)
        return res

    def angles_to_fractions(self,res):
        """
        Converts posterior samples on angles to model fractions
    ...
    def calculate_log_hierachical_likelihood(self,lambdasquare_dict,scale_free):
        """
        ...
        if scale_free:
            toret=_np.sum(_logsumexp(self.loglij+new_log_priors_matrix,axis=1)-_logsumexp(self.logsj+new_log_priors))
        else:
            toret=_np.sum(_logsumexp(self.loglij+new_log_priors_matrix,axis=1)-_logsumexp(self.logsj+new_log_priors))+logNexp*Nobs-_np.exp(logNexp)
...
class _mixture_bilby_likeli(_bilby.Likelihood):
    ...
    prior_mode:  string or dict
        If 'isotropic' we will assume isotropic model in sqrt(mixing factors). Note that this model preferes poles.
        if 'homogeneous' the density of mixing factors is constant and no model is preferred.
        otherwise (if dict) we will assume that the models does not sum to unity
    """
    def __init__(self,mixture_analyis,prior_mode,scale_free):
    ...
    def log_likelihood(self):
        ...
        if self.prior_mode=='dirichlet':
            ...
            alphs=[self.parameters['alpha_syn'+str(i)] for i in range(len(self.mixture_analyis.syn_names))]
            log_correction=_dirichlet.logpdf(lamb,alphs)-_dirichlet.logpdf(lamb,_np.ones_like(alphs)*0.5)

        elif self.prior_mode=='free':
            lambdas={key: self.parameters[key] for key in self.parameters.keys()}
            log_correction=0.

        else:
            raise ValueError('Prior mode not known')

        return self.mixture_analyis.calculate_log_hierachical_likelihood(lambdas,self.scale_free)+log_correction
