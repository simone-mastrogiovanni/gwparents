...
class list_of_populations(object):
    ...
    def calculate_log_pmodel_given_theta(self,bins,Nexp=None):
        ...
        """
        self.pmodel_vars=self.binary_vars
        minmax=self.minmax
        edges={}
        if isinstance(bins[self.pmodel_vars[0]],_np.ndarray):
            for i,var in enumerate(self.pmodel_vars):
                edges[var]=bins[var]
        else:
            for i,var in enumerate(self.pmodel_vars):
        ...
        **kwargs: Any kwargs you would pass to corner.corner
        ...
        for key in self.population_list.keys():
            if self.population_list[key].name ==name:
                toplot = _np.column_stack([self.population_list[key].binary_params[var]['values'][self.population_list[key].detlabels] for var in self.binary_vars])
                labels = [self.population_list[key].binary_params[var]['tex_label'] for var in self.binary_vars]
                ran=[(self.minmax[var]['min'],self.minmax[var]['max']) for var in self.binary_vars]
    ...
    def plot_histograms_astrophysical(self,name,**kwargs):
        ...
        **kwargs: Any kwargs you would pass to corner.corner
        ...
        for key in self.population_list.keys():
            if self.population_list[key].name ==name:
                toplot = _np.column_stack([self.population_list[key].binary_params[var]['values'] for var in self.binary_vars])
                labels = [self.population_list[key].binary_params[var]['tex_label'] for var in self.binary_vars]
                ran=[(self.minmax[var]['min'],self.minmax[var]['max']) for var in self.binary_vars]
                bin=[len(self.common_edges[jax]) for jax in range(len(self.common_edges))]
                return _corner.corner(toplot,bins=bin,range=ran,labels=labels,**kwargs)
...
class synthetic_population(object):
    ...
    def generate_delta_like_injections(self,outdir,Ngen,sigma,Nsamp=5000,mode='uniform'):
        ...
        for i in range(Ngen):
            ...
            for var in self.binary_vars:
                ...
                if mode == 'uniform':
                    if val==0:
                        valnoise=_np.random.uniform(low=-sigma[var],high=sigma[var],size=1)[0]
                        num=_np.random.uniform(low=valnoise-sigma[var],high=valnoise+sigma[var],size=Nsamp)
                    else:
                        valnoise=_np.random.uniform(low=val*(1.-sigma[var]),high=val*(1.+sigma[var]),size=1)[0]
                        num=_np.random.uniform(low=valnoise-val*sigma[var],high=valnoise+val*sigma[var],size=Nsamp)
                elif mode == 'gaussian':
                    if val==0:
                        valnoise=_np.random.randn(1)*sigma[var]
                        num=_np.random.randn(Nsamp)*sigma[var]+valnoise
                    else:
                        valnoise=_np.random.randn(1)*val*sigma[var]+val
                        num=_np.random.randn(Nsamp)*val*sigma[var]+valnoise
    ...
    def apply_selection_cut(self,ilabel_det):
        ...
        ----------
        ilabel_det: nd.array
            Label array TRUE when the event is detected
        """
